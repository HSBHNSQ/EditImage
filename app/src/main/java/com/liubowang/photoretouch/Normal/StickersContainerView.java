...
public class StickersContainerView extends RelativeLayout {
    ...
    public void setCurrentStickerViewSelected(boolean selected){
        if (currentActiveStickerView != null){
            currentActiveStickerView.setShowDrawController(selected);
        }
    }

    public void clearAllStickers(){
        currentActiveStickerView = null;
        for (StickerView sv: stickerViewList){
            removeView(sv);
        }
        stickerViewList.clear();
    }
