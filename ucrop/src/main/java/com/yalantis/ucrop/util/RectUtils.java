...
public class RectUtils {
    ...
    public static boolean leftOfLine(Point pointC,Point lineStartPointA, Point lineEndPointB){
        Point AC = new Point(pointC.x-lineStartPointA.x,
                             pointC.y - lineStartPointA.y);
        Point BC = new Point(pointC.x-lineEndPointB.x,
                             pointC.y - lineEndPointB.y);
//        计算自身和矢量v的叉乘(按照右手定则)
//        顺时针为正，逆时针为负, 等于0表示完两矢量夹角为180/0度
        int result = AC.x * BC.y - AC.y * BC.y;
        return result > 0;
    }
