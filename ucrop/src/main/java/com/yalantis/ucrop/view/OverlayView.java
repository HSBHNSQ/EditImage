...
public class OverlayView extends View {
    ...
    public static final int FREESTYLE_CROP_MODE_ENABLE_WITH_ASPECT_RATIO = 3;
        ...
        if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_MOVE) {
            if (event.getPointerCount() == 1 && mCurrentTouchCornerIndex != -1) {
                ...
                if (mFreestyleCropMode == FREESTYLE_CROP_MODE_ENABLE_WITH_ASPECT_RATIO){
                    updateCropViewRectKeepAspectRatio(x,y);
                }else {
     ...
     * 3<-------2
     */
    private void updateCropViewRectKeepAspectRatio(float touchX, float touchY) {
        mTempRect.set(mCropViewRect);
        Point C = new Point((int) touchX,(int) touchY);

        switch (mCurrentTouchCornerIndex) {
            // resize rectangle
            case 0:
            {
                float left = touchX,top = touchY;
                float height = mCropViewRect.bottom - touchY;
                float width = height * (mCropViewRect.width() / mCropViewRect.height());
                left = mCropViewRect.right - width;
                if (left > 0){
                    mTempRect.set(left, top, mCropViewRect.right, mCropViewRect.bottom);
                }
            ...
            case 1:
            {
                float right = touchX, top = touchY;
                float height = mCropViewRect.bottom - touchY;
                float width = height * (mCropViewRect.width() / mCropViewRect.height());
                right = mCropViewRect.left + width;
                if (right < getRight()){
                    mTempRect.set(mCropViewRect.left, top, right, mCropViewRect.bottom);
                }
            }
                break;
            case 2:
            {
                float right = touchX, bottom = touchY;
                float height = touchY - mCropViewRect.top;
                float width = height * (mCropViewRect.width() / mCropViewRect.height());
                right = mCropViewRect.left + width;
                if (right < getRight()){
                    mTempRect.set(mCropViewRect.left, mCropViewRect.top, right, bottom);
                }
            }

                break;
            case 3:
                float left = touchX, bottom = touchY;
                float height = touchY - mCropViewRect.top;
                float width = height * (mCropViewRect.width() / mCropViewRect.height());
                left = mCropViewRect.right - width;
                if (left > 0){
                    mTempRect.set(left, mCropViewRect.top, mCropViewRect.right, bottom);
                }
                break;
            // move rectangle
            case 4:
        ...
        boolean change = changeHeight || changeWidth;
        mCropViewRect.set(
                change ? mTempRect.left : mCropViewRect.left,
                change ? mTempRect.top : mCropViewRect.top,
                change ? mTempRect.right : mCropViewRect.right,
                change ? mTempRect.bottom : mCropViewRect.bottom);
...
//        }
    ...
    @IntDef({FREESTYLE_CROP_MODE_DISABLE, FREESTYLE_CROP_MODE_ENABLE, FREESTYLE_CROP_MODE_ENABLE_WITH_PASS_THROUGH,FREESTYLE_CROP_MODE_ENABLE_WITH_ASPECT_RATIO})
